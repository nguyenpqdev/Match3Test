...
public class Board
{
    ...
    private Dictionary<int, List<Cell>> matchCache = new Dictionary<int, List<Cell>>();
    private int cacheKey = 0;

    private int GetCacheKey(Cell cell)
    {
        return cell.BoardX * 1000 + cell.BoardY;
    }
    ...
    {
        int key = GetCacheKey(cell) + 1000000; // Offset to avoid collision with horizontal cache
        if (matchCache.ContainsKey(key))
        {
            return matchCache[key];
        }
    ...
    internal void ShiftDownItems()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            ...
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                holder.Assign(item);
                shiftSequence.Join(item.View.DOMove(holder.transform.position, 0.3f));
            }
        }
    }
